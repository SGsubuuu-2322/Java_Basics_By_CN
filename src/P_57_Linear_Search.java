...
import java.util.*;
public class P_57_Linear_Search {
	
	public static int[] inputArray(){
		Scanner ip = new Scanner(System.in);
		System.out.println("So, enter the length of an array :- ");
		int len = ip.nextInt();
		int [] arr = new int[len];
		for(int i = 0; i < arr.length; i++){
			System.out.println("Now, enter the element for " + i + "th index :- ");
			arr[i] = ip.nextInt();
		}
		return arr;
	}
	...
	public static void main(String[] args) {
		Scanner ip = new Scanner(System.in);
		System.out.println("This is the program for creating an array and searching a number in that array...");
		int [] arr = inputArray();
		System.out.println("Now, enter the number you want to search in that array :- ");
		int num = ip.nextInt();
